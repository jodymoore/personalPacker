...
//  MyContacts
//
//  Created by Jody Moore on 2016-05-05
...
class CustomerTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    //**Begin Copy**
    ...
    func updateSearchResults(for searchController: UISearchController)
    {
        ...
        let searchPredicate = NSPredicate(format: "customername CONTAINS[c] %@", searchController.searchBar.text!)
        let array = (customerArray as NSArray).filtered(using: searchPredicate)
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            if (self.resultSearchController.isActive) {
                ...
                context.delete(customerArray[(indexPath as NSIndexPath).row])
            }
            ...
            if let destination = segue.destination as?
                ...
                if (self.resultSearchController.isActive) {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        ...
                        destination.Customerdb = selectedDevice
                        resultSearchController.isActive = false
                    }
                }
                else {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        let selectedDevice:NSManagedObject = customerArray[SelectIndex] as NSManagedObject
                        destination.Customerdb = selectedDevice
                    }
                }
            }
